...
async def scan_receipt_with_ai(image_bytes: bytes) -> str:
    ...
    system_prompt = (
        "Ты распознаёшь чеки. Отвечай ТОЛЬКО на русском языке.\n"
        "Найди ИТОГОВУЮ сумму чека и верни ОДНУ строку строго в формате:\n"
        "TRANSACTION: -[итоговая_сумма] [категория] бн\n\n"
        "Категории: Еда/Продукты, Транспорт, Здоровье, Одежда, Развлечения, Прочие расходы\n"
        "Если оплата наличными — пиши нал вместо бн.\n\n"
        "ВАЖНО: никакого текста кроме строки TRANSACTION. Никаких пояснений. Только одна строка.\n\n"
        "Пример ответа:\n"
        "TRANSACTION: -1112.53 Еда/Продукты бн"
    )
...
async def msg_photo_receipt(message: Message):
    ...
    try:
        ...
        await message.answer("GPT ответ: " + result[:200])
