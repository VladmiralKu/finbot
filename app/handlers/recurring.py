...
async def cb_calendar(call: CallbackQuery):
    ...
    intro = (
        "🗓 Платёжный календарь\n\n"
        "Сюда добавляют будущие и регулярные платежи: аренду, кредиты, подписки, налоги.\n"
        "Они попадают в планируемые расходы ДДС и помогают видеть свободный остаток заранее.\n\n"
        "Добавить можно кнопкой ниже или через ИИ-помощника: «добавь аренду 40000 каждый месяц 5 числа».\n"
        "Удалить — кнопкой «Удалить платёж» или фразой в ИИ-помощнике.\n\n"
    )

    if not payments:
        text = intro + "У тебя пока нет регулярных платежей.\n\nДобавь первый — и я буду напоминать тебе о нём!"
    else:
        text = intro
        for p in payments:
            next_date = p[14]
